...
async function view(viewName, options) {
  ...
  const { api, route, component, store } = options;
  ...
  // 创建组件目录
  if (component) {
    createComponent(viewName);
  }

  // 创建路由文件
  if (route) {
    createRoute(viewName);
  }
...
async function createView(viewName) {
  ...
  if (exist) {
    console.log(warning('检测到已存在该目录，取消创建'));
    return false;
  }
...
async function createComponent(name) {
  ...
  const exist = fs.existsSync(fileFolder);
  if (exist) {
    console.log(warning('检测到已存在该组件目录，取消创建'));
    return false;
  }
...
async function createRoute(name) {
  ...
  if (exist) {
    console.log(warning('检测到已存在该路由文件，取消创建'));
    return false;
  }
...
async function createStore(name) {
  ...
  const exist = fs.existsSync(fileName);
  if (exist) {
    console.log(warning('检测到已存在该状态文件，取消创建'));
    return false;
  }
...
async function createApi(name) {
  const fileFolder = path.resolve(cwd, `src/api`);
  const fileName = path.resolve(fileFolder, `${name}.js`);
  const exist = fs.existsSync(fileName);
  if (exist) {
    console.log(warning('检测到已存在该接口文件，取消创建'));
    return false;
  }
  logWithSpinner('⏰ 正在创建接口文件......');
  shell.mkdir('-p', fileFolder);
  shell.touch(fileName);
  const { api } = require('./template');
  fs.writeFileSync(fileName, api);
  logWithSpinner('⏰ 创建接口文件成功......');
  stopSpinner(true);
  return true;
}
